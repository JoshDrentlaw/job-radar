...
`sweep`, and a read-only `digest`. These routes authenticate with a token, never the session
cookie — a token here grants nothing on the rest of the app, and the session cookie grants nothing
here.
...
- **Revoke** disables a token immediately. The recommended order when rotating: mint the
  replacement, paste it into n8n, *then* revoke the old one — that way the scheduler is never
  left without a working credential in between.
...
remaining — call again), **500** the run itself failed. Treat it as three states, not a
pass/fail binary.
