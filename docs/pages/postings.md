...
Every job pulled from every registered board, unfiltered by anything the app has an opinion about.
This is the raw feed — for "which of these fit me," see [Matches](matches.md) instead; this page's
search is deliberately plain keyword matching over title and description, not the semantic skill
matching that page does.
